...
* *wd Integration*: Automatically creates shortcuts using the `wd` command for spring-prefixed owners and repositories (if `wd` is available)
...
5. *wd Shortcuts* (if `wd` command is available):
   - If the owner starts with `spring-`, creates a shortcut with the prefix removed
     * Example: `spring-projects` → `wd projects` points to `~/code/spring-projects`
   - If the repository starts with `spring-`, creates a shortcut with the prefix removed
     * Example: `spring-security` → `wd security` points to `~/code/spring-projects/spring-security/main`
...
* Create wd shortcuts (if `wd` is available):
  - `wd projects` → `~/code/spring-projects`
  - `wd security` → `~/code/spring-projects/spring-security/main`
