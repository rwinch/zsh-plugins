...
A zsh plugin that provides a smart `clone` function for cloning Git repositories with intelligent directory management and integration with `git-url`.
...
* *Automatic Directory Change*: Changes to the cloned repository directory after successful clone
...
# Load git-clone
antigen bundle rwinch/zsh-plugins git-clone
----

*Important*: The `git-url` plugin must be loaded before `git-clone`.

=== Manual Installation

1. Clone or download both the `git-url` and `git-clone` plugins
2. Source the plugin files in your `.zshrc`:

[source,bash]
----
source /path/to/git-url/git-url.plugin.zsh
source /path/to/git-clone/git-clone.plugin.zsh
----
