...
antigen bundle rwinch/zsh-plugins gh-create-subissue
antigen bundle rwinch/zsh-plugins gh-update
antigen bundle rwinch/zsh-plugins get-security-module-names
antigen bundle rwinch/zsh-plugins git-recent
antigen bundle rwinch/zsh-plugins gw

# Load the theme (use rdubbs to show git repo path in the prompt)
...
After making changes to your `.antigenrc` or adding new plugins, reload them with:
...
The `antigen reset` command clears Antigen's cache and forces a fresh load of all bundles.
